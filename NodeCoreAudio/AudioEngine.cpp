...
Audio::AudioEngine::AudioEngine(Local<Object>options) :
  ...
  for (int iChannel = 0; iChannel < m_uInputChannels;
       iChannel++) m_hInputBuffer->Set(iChannel,
                                       Nan::New<Array>(m_uSamplesPerBuffer));
  ...
  if (initErr != paNoError) Nan::ThrowTypeError(
  ...
  if (openStreamErr != paNoError) Nan::ThrowTypeError(
      "Failed to open audio stream");

  // Start the audio stream
  PaError startStreamErr = Pa_StartStream(m_pPaStream);

  if (startStreamErr != paNoError) Nan::ThrowTypeError(
...
/*! Gets options */
void Audio::AudioEngine::getOptions(
  const Nan::FunctionCallbackInfo<v8::Value>& info) {
  Local<Object> options = Nan::New<Object>();

  AudioEngine *pEngine = AudioEngine::Unwrap<AudioEngine>(info.This());

  Nan::Set(options, Nan::New<String>(
             "inputChannels").ToLocalChecked(),
                                               Nan::New<Number>(pEngine->
                            m_uInputChannels));
  Nan::Set(options, Nan::New<String>(
             "outputChannels").ToLocalChecked(),
                                               Nan::New<Number>(pEngine->
                            m_uOutputChannels));

  Nan::Set(options, Nan::New<String>(
             "inputDevice").ToLocalChecked(),
                                               Nan::New<Number>(pEngine->
                            m_uInputDevice));
  Nan::Set(options, Nan::New<String>(
             "outputDevice").ToLocalChecked(), Nan::New<Number>(
             pEngine->m_uOutputDevice));

  Nan::Set(options, Nan::New<String>(
             "sampleRate").ToLocalChecked(),
                                               Nan::New<Number>(pEngine->
                            m_uSampleRate));
  Nan::Set(options, Nan::New<String>(
             "sampleFormat").ToLocalChecked(), Nan::New<Number>(
             pEngine->m_uSampleFormat));
  Nan::Set(options, Nan::New<String>(
             "framesPerBuffer").ToLocalChecked(),
                                               Nan::New<Number>(pEngine->
                            m_uSamplesPerBuffer));
  Nan::Set(options, Nan::New<String>(
             "numBuffers").ToLocalChecked(),
                                               Nan::New<Number>(pEngine->
                            m_uNumBuffers));
  Nan::Set(options, Nan::New<String>(
             "interleaved").ToLocalChecked(),  Nan::New<Boolean>(
             pEngine->m_bInterleaved));
  Nan::Set(options, Nan::New<String>(
             "useMicrophone").ToLocalChecked(),
                                               Nan::New<Boolean>(pEngine->
                             m_bReadMicrophone));
...
void Audio::AudioEngine::setOptions(
  const Nan::FunctionCallbackInfo<v8::Value>& info) {
...
void Audio::AudioEngine::applyOptions(Local<Object>options) {
  ...
  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("inputDevice").ToLocalChecked()).
      FromMaybe(false))
    m_uInputDevice =
      Nan::To<int>(Nan::Get(options,
                            Nan::New<String>(
                              "inputDevice").ToLocalChecked()).ToLocalChecked()).
      FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("outputDevice").ToLocalChecked()).
      FromMaybe(false))
    m_uOutputDevice =
      Nan::To<int>(Nan::Get(options,
                            Nan::New<String>(
                              "outputDevice").ToLocalChecked()).ToLocalChecked()).
      FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("inputChannels").ToLocalChecked()).
      FromMaybe(false))
    m_uInputChannels =
      Nan::To<int>(Nan::Get(options,
                            Nan::New<String>(
                              "inputChannels").ToLocalChecked()).ToLocalChecked())
      .FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("outputChannels").ToLocalChecked()).
      FromMaybe(false))
    m_uOutputChannels =
      Nan::To<int>(Nan::Get(options,
                            Nan::New<String>(
                              "outputChannels").ToLocalChecked()).ToLocalChecked())
      .
      FromJust();

  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("framesPerBuffer").ToLocalChecked()).
      FromMaybe(false))
    m_uSamplesPerBuffer =
      Nan::To<int>(Nan::Get(options,
                            Nan::New<String>(
                              "framesPerBuffer").ToLocalChecked()).ToLocalChecked())
      .
      FromJust();

  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("numBuffers").ToLocalChecked()).
      FromMaybe(false)) m_uNumBuffers
      = Nan::To<int>(Nan::Get(options,
                              Nan::New<String>(
                                "numBuffers").ToLocalChecked()).ToLocalChecked()).
        FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("interleaved").ToLocalChecked()).
      FromMaybe(false))
    m_bInterleaved =
      Nan::To<bool>(Nan::Get(options,
                             Nan::New<String>(
                               "interleaved").ToLocalChecked()).ToLocalChecked()).
      FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("useMicrophone").ToLocalChecked()).
      FromMaybe(false))
    m_bReadMicrophone =
      Nan::To<bool>(Nan::Get(options,
                             Nan::New<String>(
                               "useMicrophone").ToLocalChecked()).ToLocalChecked())
      .FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("sampleRate").ToLocalChecked()).
      FromMaybe(false)) m_uSampleRate
      = Nan::To<int>(Nan::Get(options,
                              Nan::New<String>(
                                "sampleRate").ToLocalChecked()).ToLocalChecked()).
        FromJust();


  if (Nan::HasOwnProperty(options,
                          Nan::New<String>("sampleFormat").ToLocalChecked()).
      FromMaybe(false)) {
    switch (Nan::To<int>(Nan::Get(options,
                                  Nan::New<String>("sampleFormat").ToLocalChecked())
  ...
  m_inputParams.suggestedLatency =
    Pa_GetDeviceInfo(m_inputParams.device)->defaultLowInputLatency;
  ...
  m_outputParams.suggestedLatency          = Pa_GetDeviceInfo(
    m_outputParams.device)->defaultLowOutputLatency;
  m_outputParams.hostApiSpecificStreamInfo = NULL;

  // Clear out our temp buffer blocks
  if (m_cachedInputSampleBlock != NULL) free(m_cachedInputSampleBlock);
  ...
  if (m_cachedOutputSampleBlockForWriting != NULL) free(
      m_cachedOutputSampleBlockForWriting);

  // Allocate some new space for our temp buffer blocks
  m_cachedInputSampleBlock  = (char *)malloc(
    m_uSamplesPerBuffer * m_uInputChannels * m_uSampleSize);
  m_cachedOutputSampleBlock = (char **)malloc(sizeof(char *) * m_uNumBuffers);

  for (int i = 0; i < m_uNumBuffers; i++) {
    m_cachedOutputSampleBlock[i] = (char *)malloc(
      m_uSamplesPerBuffer * m_uOutputChannels * m_uSampleSize);
  }
  m_cachedOutputSampleBlockForWriting = (char *)malloc(
    m_uSamplesPerBuffer * m_uOutputChannels * m_uSampleSize);
  m_uNumCachedOutputSamples           = (unsigned int *)calloc(
    sizeof(unsigned int),
    m_uNumBuffers);

  if ((m_pPaStream != NULL) && Pa_IsStreamActive(m_pPaStream)) restartStream();
...
Local<Array>Audio::AudioEngine::getInputBuffer() {
  ...
  if (m_bInterleaved) {
    ...
    for (int iSample = 0; iSample < m_uSamplesPerBuffer * m_uInputChannels;
         ++iSample) {
...
void Audio::AudioEngine::setSample(int position, Handle<Value>sample) {
  ...
  switch (m_uSampleFormat) {
  case paFloat32:
    ((float *)m_cachedOutputSampleBlock[m_uCurrentWriteBuffer])[position] =
      (float)sample->NumberValue();
    break;

  case paInt32:
    ((int *)m_cachedOutputSampleBlock[m_uCurrentWriteBuffer])[position] =
      (int)sample->NumberValue();
    break;

  case paInt24:
    temp =
      (int)sample->NumberValue();
    m_cachedOutputSampleBlock[m_uCurrentWriteBuffer][3 * position + 0] = temp >>
                                                                         16;
    m_cachedOutputSampleBlock[m_uCurrentWriteBuffer][3 * position + 1] = temp >>
                                                                         8;
    m_cachedOutputSampleBlock[m_uCurrentWriteBuffer][3 * position + 2] = temp;
    break;

  case paInt16:
    ((int16_t *)m_cachedOutputSampleBlock[m_uCurrentWriteBuffer])[position] =
      (int16_t)sample->NumberValue();
    break;

  default:
    m_cachedOutputSampleBlock[m_uCurrentWriteBuffer][position] =
      (char)sample->NumberValue();
    break;
  }
...
void Audio::AudioEngine::queueOutputBuffer(Handle<Array>result) {
  ...
  if (m_bInterleaved) {
    for (int iSample = 0; iSample < m_uSamplesPerBuffer * m_uOutputChannels;
         ++iSample) setSample(iSample, result->Get(iSample));

    m_uNumCachedOutputSamples[m_uCurrentWriteBuffer] = result->Length() /
                                                       m_uOutputChannels;
  } else {
    // Validate the structure of the output buffer array
    if (!result->Get(0)->IsArray()) {
      Nan::ThrowTypeError(
        "Output buffer not properly setup, 0th channel is not an array");
      return;
    }
    ...
    for (int iChannel = 0; iChannel < m_uOutputChannels; ++iChannel) {
      for (int iSample = 0; iSample < m_uSamplesPerBuffer; ++iSample) {
        ...
        if (item->IsArray()) {
          if (item->Length() >
              m_uNumCachedOutputSamples[m_uCurrentWriteBuffer])
            m_uNumCachedOutputSamples
            [m_uCurrentWriteBuffer] = item->Length();
...
void Audio::AudioEngine::RunAudioLoop() {
  ...
  while (true) {
    ...
    if (m_uNumCachedOutputSamples[m_uCurrentReadBuffer] > 0) {
      memcpy(m_cachedOutputSampleBlockForWriting,
             m_cachedOutputSampleBlock[m_uCurrentReadBuffer],
             m_uNumCachedOutputSamples[m_uCurrentReadBuffer] * m_uOutputChannels *
             m_uSampleSize);
      numSamples =
        m_uNumCachedOutputSamples[m_uCurrentReadBuffer];
      m_uNumCachedOutputSamples[m_uCurrentReadBuffer] = 0;
      m_uCurrentReadBuffer                            =
        (m_uCurrentReadBuffer + 1) % m_uNumBuffers;
    }
    uv_mutex_unlock(&m_mutex);

    if (numSamples > 0) {
      error                = Pa_WriteStream(m_pPaStream,
                                            m_cachedOutputSampleBlockForWriting,
                                            numSamples);
      ...
      Sleep(1);
#else // if defined(__WINDOWS__) || defined(_WIN32)
      sleep(1);
#endif // if defined(__WINDOWS__) || defined(_WIN32)
    }
  }
...
NAN_MODULE_INIT(Audio::AudioEngine::Init) {
  ...
  // target->Set(Nan::New<String>("AudioEngine"),
  // functionTemplate->GetFunction());

  // Get
  // functionTemplate->PrototypeTemplate()->Set( Nan::New<String>("isActive"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::isActive)->GetFunction() );
  // functionTemplate->PrototypeTemplate()->Set(
  // Nan::New<String>("getDeviceName"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::getDeviceName)->GetFunction()
  // );
  // functionTemplate->PrototypeTemplate()->Set(
  // Nan::New<String>("getNumDevices"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::getNumDevices)->GetFunction()
  // );
  Nan::SetPrototypeMethod(functionTemplate,
                          "isActive",
                          Audio::AudioEngine::isActive);
  Nan::SetPrototypeMethod(functionTemplate,
                          "getDeviceName",
                          Audio::AudioEngine::getDeviceName);
  Nan::SetPrototypeMethod(functionTemplate,
                          "getNumDevices",
                          Audio::AudioEngine::getNumDevices);

  // Set
  // functionTemplate->PrototypeTemplate()->Set( Nan::New<String>("setOptions"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::setOptions)->GetFunction()
  // );
  // functionTemplate->PrototypeTemplate()->Set( Nan::New<String>("getOptions"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::getOptions)->GetFunction()
  // );
  // functionTemplate->PrototypeTemplate()->Set( Nan::New<String>("write"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::write)->GetFunction() );
  // functionTemplate->PrototypeTemplate()->Set( Nan::New<String>("read"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::read)->GetFunction() );
  // functionTemplate->PrototypeTemplate()->Set(
  // Nan::New<String>("isBufferEmpty"),
  // Nan::New<FunctionTemplate>(Audio::AudioEngine::isBufferEmpty)->GetFunction()
  // );
  Nan::SetPrototypeMethod(functionTemplate,
                          "setOptions",
                          Audio::AudioEngine::setOptions);
  Nan::SetPrototypeMethod(functionTemplate,
                          "getOptions",
                          Audio::AudioEngine::getOptions);
  Nan::SetPrototypeMethod(functionTemplate,
                          "write",
                          Audio::AudioEngine::write);
  Nan::SetPrototypeMethod(functionTemplate,
                          "read",
                          Audio::AudioEngine::read);
  Nan::SetPrototypeMethod(functionTemplate,
                          "isBufferEmpty",
                          Audio::AudioEngine::isBufferEmpty);
...
// v8::Handle<v8::Value> Audio::AudioEngine::NewInstance(const v8::Arguments&
// info) {
void Audio::AudioEngine::NewInstance(
  const Nan::FunctionCallbackInfo<v8::Value>& info) {
  Nan::HandleScope scope;
...
void Audio::AudioEngine::New(const Nan::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (info.Length() > 0) {
    if (!info[0]->IsObject()) return Nan::ThrowTypeError(
        "First argument must be an object.");
    else    options = Local<Object>::Cast(info[0]);
  } else {
...
void Audio::AudioEngine::isBufferEmpty(
  const Nan::FunctionCallbackInfo<v8::Value>& info) {
  Nan::HandleScope scope;
  ...
  Local<Boolean> isEmpty =
    Nan::New<Boolean>(pEngine->m_uNumCachedOutputSamples[pEngine->
                                                         m_uCurrentWriteBuffer] ==
                      0);
  uv_mutex_unlock(&pEngine->m_mutex);
...
void Audio::AudioEngine::isActive(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
  ...
  if (Pa_IsStreamActive(pEngine->m_pPaStream)) info.GetReturnValue().Set(Nan::New<
                                                                           Boolean>(
                                                                           true));

  else    info.GetReturnValue().Set(Nan::New<Boolean>(false));
} // end AudioEngine::IsActive()

//////////////////////////////////////////////////////////////////////////////

/*! Get the name of an audio device with a given ID number */
void Audio::AudioEngine::getDeviceName(
  const Nan::FunctionCallbackInfo<v8::Value>& info) {
  Nan::HandleScope scope;
  ...
  const PaDeviceInfo *pDeviceInfo = Pa_GetDeviceInfo(
    (PaDeviceIndex)deviceIndex->NumberValue());
...
void Audio::AudioEngine::getNumDevices(
  const Nan::FunctionCallbackInfo<v8::Value>& info) {
  Nan::HandleScope scope;
...
void Audio::AudioEngine::restartStream() {
  ...
  if (error != paNoError) Nan::ThrowTypeError("Failed to stop audio stream");

  // Close the audio stream
  error = Pa_CloseStream(m_pPaStream);

  if (error != paNoError) Nan::ThrowTypeError("Failed to close audio stream");
  ...
  if (error != paNoError) Nan::ThrowTypeError("Failed to start audio stream");
...
Audio::AudioEngine::~AudioEngine() {
  ...
  if (err !=
      paNoError) fprintf(stderr, "PortAudio error: %s\n", Pa_GetErrorText(err));
